...
module.exports = function (api_key, api_version) {
  var auth = 'Basic ' + new Buffer(api_key + ':').toString('base64');

  api_version = api_version || 'v2.1';
  ...
  return {
    ...
    subscriptions: {
      ...
      remove: function(subscription_id, data, cb) {
        if (typeof subscription_id !== 'string') {
          throw new TypeError('subscription_id must be a string');
        }
        ...
        del('/subscriptions/' + subscription_id, data, cb);
      },
      list: function(data, cb) {
